  ...
  if (currentModuleIsInRootDirectory) {
    ...
    if (currentModuleLevel) {
      ...
      if (targetModuleLevel) {
        ...
        console.log(currentModuleLevelConfiguration); //луда
        console.log(configurationOfTargetModule); //откуда если указали
        console.log(parentTargetModule); // учли не указали
        if (configurationOfTargetModule) {
          if (currentModuleLevelConfiguration.parents === configurationOfTargetModule.parents) {
           ...
           if (currentModuleLevelConfiguration.firstParent !== configurationOfTargetModule.firstParent) {
            const parentCurrentModuleLevelConfiguration = configurationTree.find(
              (elem) => elem.name === currentModuleLevelConfiguration.firstParent
            );
            // console.log(parentCurrentModuleLevelConfiguration);
            const parentConfigurationOfTargetModule = configurationTree.find(
              (elem) => elem.name === configurationOfTargetModule.firstParent
            );
            console.log(parentConfigurationOfTargetModule);
            if (parentCurrentModuleLevelConfiguration.index <= parentConfigurationOfTargetModule.index) {
          ...
          if (currentModuleLevelConfiguration.firstParent !== parentTargetModule.firstParent) {
             //console.log(1);
            const parentCurrentModuleLevelConfiguration = configurationTree.find(
              (elem) => elem.name === currentModuleLevelConfiguration.firstParent
            );
            //console.log(parentCurrentModuleLevelConfiguration);
            const parentConfigurationOfTargetModule = configurationTree.find(
              (elem) => elem.name === parentTargetModule.firstParent
            );
           // console.log(parentConfigurationOfTargetModule);
            if (parentCurrentModuleLevelConfiguration.index <= parentConfigurationOfTargetModule.index) {
              return `import prohibited due to different nesting levels`;
